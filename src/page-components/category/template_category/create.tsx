...
import { createTemplateCategory, listTemplateCategory } from 'api'
...
const Create: React.FC<Props> = ({ onOk, onCancel }) => {
  ...
  const categories = useRequest<Category[]>(listTemplateCategory)
  ...
  const createAction = useRequest(createTemplateCategory, {
    manual: true,
    onSuccess: () => {
