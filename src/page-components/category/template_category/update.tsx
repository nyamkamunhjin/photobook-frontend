...
import { getTemplateCategory, listTemplateCategory, updateTemplateCategory } from 'api'
...
const Update: React.FC<Props> = ({ id, onOk, onCancel }) => {
  ...
  const getAction = useRequest<ImageCategory>(() => getTemplateCategory(id))
  const categories = useRequest<ImageCategory[]>(() => listTemplateCategory)
  const updateAction = useRequest(updateTemplateCategory, {
    manual: true,
    onSuccess: () => {
