...
import { createImageCategory, listImageCategory } from 'api'
...
const key = 'create_category'

const Create: React.FC<Props> = ({ onOk, onCancel }) => {
  const intl = useIntl()
  const [form] = Form.useForm()
  const categories = useRequest<ImageCategory[]>(() => listImageCategory('backgrounds'))
  ...
  const createAction = useRequest(createImageCategory, {
    manual: true,
    onSuccess: () => {
  ...
  return (
    <FormModal
      ...
      onFinish={(values) => {
        ...
        createAction.run({ ...values, type: 'backgrounds' })
      }}
    >
      <Row>
