...
import { getLayoutCategory, listLayoutCategory, updateLayoutCategory } from 'api'
...
const Update: React.FC<Props> = ({ id, onOk, onCancel }) => {
  ...
  const getAction = useRequest<ImageCategory>(() => getLayoutCategory(id))
  const categories = useRequest<ImageCategory[]>(() => listLayoutCategory())
  const updateAction = useRequest(updateLayoutCategory, {
    manual: true,
    onSuccess: () => {
