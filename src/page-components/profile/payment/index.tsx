...
const PaymentTypes: React.FC<Props> = ({ payment, close, id }) => {
  ...
  return (
    <>
      <Form form={form} name="pay" className="align_center" layout="vertical" labelAlign="left" onFinish={handleFinish}>
          ...
          <Radio.Group className="w-fill" onChange={() => refresh.toggle()}>
            <Row>
              <Col xs={24} sm={24} md={24} lg={12}>
                <Radio.Button value="personal" className={styles.radio}>
                  <IconText
                    bank="personal"
              ...
              <Col xs={24} sm={24} md={24} lg={12}>
                <Radio.Button value="organization" className={styles.radio}>
                  <IconText
                    bank="organization"
        ...
        {form.getFieldValue('ebarimt_type') === 'organization' && (
          <Form.Item
            ...
            className={styles.organization_code}
            ...
            <Row>
              {payment.types?.map((type) => (
                <Col key={type.id} xs={24} sm={24} md={24} lg={12}>
                  <Radio.Button value={type.name} className={styles.radio}>
        ...
        <Row className={styles.buttonField}>
          <Col span={6} xs={11} sm={11} md={6}>
            <Button block onClick={() => close()} className={styles.button}>
              <FormattedMessage id="billing.back" />
            </Button>
          </Col>
          <Col span={6} xs={11} sm={11} md={6}>
            <Button block form="pay" type="primary" htmlType="submit" className={styles.button}>
