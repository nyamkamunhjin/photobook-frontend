...
export const withMaxItems = <T, P extends Props>(
  ...
  return class ComponentWithMaxItems extends React.Component<Omit<P, Props> & WithMaxItemsProps, WithMaxItemsState> {
    ...
    UNSAFE_componentWillReceiveProps(nextProps: any) {
      if (nextProps.refreshing !== this.props.refreshing) {
        this.setState({
          ...
          chunks: splitItems(nextProps.maxItems, this.props.items, createId),
        })
      }
    }
    ...
    public render = () => {
      const { items, onDragEnd, ...rest } = this.props
      return <Component {...(rest as unknown as P)} chunks={this.state.chunks} onDragEnd={this.onDragEnd} />
    }
  }
}
