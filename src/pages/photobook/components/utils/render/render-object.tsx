...
import { SlideObject, StyleType } from 'interfaces'
...
const renderObject: React.FC<Props> = ({ object, updateObject, updateHistory, saveObjects, scale }) => {
  const { props } = object
  ...
  if (props.className === 'image-placeholder') {
    ...
    return (
      <Text textStyle={textStyle} autogrowStyle={autogrowStyle} texts={texts} style={style} className={className} />
    )
