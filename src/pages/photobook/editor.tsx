...
import { Container, DomType, HistoryProps, LayoutObject, Size, Slide } from 'interfaces'
...
import { Header, BackgroundImages, SideListTools, SideButtons, SideBarPanel, Toolbar } from './components/layout'
...
interface Props {
  ...
  deleteSlide: (projectId: number, slideId: string) => Promise<void>
...
const BookEditor: React.FC<Props> = ({
  ...
  // methods
  const changeLayout = (align: string, type: string) => {
    if (type === 'less') {
      if (align === 'left') {
        const leftObjects = objects.filter((o: any) => o.style.left < slideWidth / 2)
        ...
        nextObjects.objects.forEach((o: any, i: number) => {
          ...
          const style = {
            top: (slideHeight * o.top) / 100,
            left: ((slideWidth / 2) * o.left) / 100,
            width: ((slideWidth / 2) * o.width) / 100,
            height: (slideHeight * o.height) / 100,
            rotateAngle: 0,
            transform: '',
            zIndex: 100 + i + '',
          }
        ...
        updateHistory(ADD_LAYOUT, {
          objects,
          layout,
        })
      } else {
        const rightObjects = objects.filter((o: any) => o.style.left >= slideWidth / 2)
        ...
        nextObjects.objects.forEach((o: any, i: number) => {
          ...
          const style = {
            top: (slideHeight * o.top) / 100,
            left: slideWidth / 2 + ((slideWidth / 2) * o.left) / 100,
            width: ((slideWidth / 2) * o.width) / 100,
            height: (slideHeight * o.height) / 100,
            rotateAngle: 0,
            transform: '',
  ...
  const onDeleteSlide = (projectId: number, slideId: string) => {
    if (_slideIndex !== 0) {
      setRefreshing.setTrue()
      changeSlideIndex(_slideIndex - 1)
      deleteSlide(projectId, slideId).then(() => {
  ...
  return fetching ? (
    ...
    <div className="AdvancedEditorWrapper">
      ...
      <div className="EditorOnePageView">
        <SideBarPanel layoutGroups={layouts} />
