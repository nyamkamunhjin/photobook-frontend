...
import ClipartCreate from 'page-components/images/clipart_images'
...
const Files: React.FC = () => {
  ...
  const listAction = useAntdTable<Image>((params, data) => listImage('cliparts', params, data, lastKey), {
    defaultPageSize: 20,
    form,
    onSuccess: (success) => {
    ...
    <>
      <Form form={form} size="small" onValuesChange={listAction.search.submit}>
        <CardTable
          ...
          label={intl.formatMessage({ id: 'clipart_files' })}
          createProps={{
            onClick: () => {
              setCreate.setTrue()
            },
