...
import FrameCreate from 'page-components/images/frame_images'
...
const Files: React.FC = () => {
  ...
  const listAction = useAntdTable<Image>((params, data) => listImage('frames', params, data, lastKey), {
    defaultPageSize: 20,
    form,
    onSuccess: (success) => {
    ...
    <>
      <Form form={form} size="small" onValuesChange={listAction.search.submit}>
        <CardTable
          ...
          label={intl.formatMessage({ id: 'frame_files' })}
          createProps={{
            onClick: () => {
              setCreate.setTrue()
            },
