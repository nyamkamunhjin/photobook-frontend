...
import MaskCreate from 'page-components/images/mask_images'
...
const Files: React.FC = () => {
  ...
  const listAction = useAntdTable<Image>((params, data) => listImage('masks', params, data, lastKey), {
    defaultPageSize: 20,
    form,
    onSuccess: (success) => {
    ...
    <>
      <Form form={form} size="small" onValuesChange={listAction.search.submit}>
        <CardTable
          ...
          label={intl.formatMessage({ id: 'mask_files' })}
          createProps={{
            onClick: () => {
              setCreate.setTrue()
            },
