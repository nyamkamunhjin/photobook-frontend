...
const PoorQuality: React.FC<{ data: any }> = ({ data: { onGoto, onHideAllSimilar, onHideThis, slide } }) => {
  return (
    <div className="p-4 border-t">
      <div className="flex gap-2">
        <span className="mt-1.5 w-1.5 h-1.5 rounded-md bg-yellow-500" />
        <h6 className="text-sm text-gray-500 font-normal">Photo is of poor quality.</h6>
      </div>
      <p className="text-xs text-gray-400 font-normal">
        {`This photo is low in resolution and might result in poor print quality in ${slide}. Please zoom out, choose a different layout, or replace the image.`}
      </p>
      <div className="flex gap-2">
        {/* <a className="flex-1 text-xs text-gray-500 font-normal underline" href="#" onClick={onGoto}>
          Go to
...
const Notices: React.FC<Props> = ({ notices }) => {
  return (
    <div className="grid" style={{ maxHeight: 'calc(100vh - 100px)' }}>
      ...
      <div className="flex-1 flex flex-col overflow-y-scroll">
        {notices.map(({ type, key, data }) => {
          switch (type) {
            ...
            case 'poor quality':
              return <PoorQuality data={data} key={key} />
            case 'empty slide':
