...
import { FrameCategoryUpdate, FrameCategoryCreate } from 'page-components/category/frame_category'
...
const Categories: React.FC = () => {
  ...
  const listAction = useAntdTable<ImageCategory>((params, data) => listImageCategory('frames', params, data, lastKey), {
    defaultPageSize: 20,
    form,
    onSuccess: (success) => {
      console.log(success)
    ...
    <>
      <Form form={form} size="small" onValuesChange={listAction.search.submit}>
        <CardTable
          ...
          label={intl.formatMessage({ id: 'frame_categories' })}
          createProps={{
            onClick: () => {
              setCreate.setTrue()
            },
