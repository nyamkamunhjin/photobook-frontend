...
import { MaskCategoryUpdate, MaskCategoryCreate } from 'page-components/category/mask_category'
...
const Categories: React.FC = () => {
  ...
  const listAction = useAntdTable<ImageCategory>((params, data) => listImageCategory('masks', params, data, lastKey), {
    defaultPageSize: 20,
    form,
    onSuccess: (success) => {
      console.log(success)
    ...
    <>
      <Form form={form} size="small" onValuesChange={listAction.search.submit}>
        <CardTable
          ...
          label={intl.formatMessage({ id: 'mask_categories' })}
          createProps={{
            onClick: () => {
              setCreate.setTrue()
            },
